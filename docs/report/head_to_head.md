...
My pipeline's LiDAR-tier output against one consumer scanning app on two of my benchmark rooms,
dimension by dimension, beating or tying on at least 70 % of shared dimensions. Cost is explicitly
not an accepted reason, and it is not my reason.
...
My benchmark rooms are the property in the supplied captures, which I cannot enter. The phone
available to me is an iPhone 15, which has no LiDAR, so even in a room I could reach I could not
produce a LiDAR-tier capture to compare against.

So the comparison the brief specifies requires two things I do not have at once: physical access to
...
## What I would do with an hour and a Pro device
...
| Dimension | Reference | Ours | magicplan | My abs error | Their abs error | Winner |
|---|---|---|---|---|---|---|

with a tie declared when my error is within 0.5 cm of theirs, plus the app version and its raw
