...
def run(capture_dir: str, out: str = "results", tier: str = "auto", drift_correction: str = "on",
    ...
    if label_rooms:
        t0 = time.time()
        try:
            from floorplan.perception.rooms import label_rooms as _label
            if tier == "lidar":
    ...
    if damage:
        t0 = time.time()
        try:
            from floorplan.perception.pipeline import damage_for_rooms
    ...
    t0 = time.time()
    try:
        from floorplan.rendering.plan import render_plan
...
@app.command()
def bench(out: str = "analysis/bench/latest", captures: str = "data/benchmark/captures", gt: str = "data/benchmark/ground_truth",
          manifest: str = "data/benchmark/bench.yaml", only: str = "", calibrate: bool = False,
          legacy: bool = False):
