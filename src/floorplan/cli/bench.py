...
def run_bench(out: str, captures_dir: str, gt_dir: str, manifest: str = "data/benchmark/bench.yaml",
    ...
    if do_calibrate:
        from floorplan.calibration.fit import fit_factors
...
def _repeatability(results):
    ...
    for b, a in pairs:
        if a not in results:
            continue
        load_plan(os.path.join(os.path.dirname(""), "")) if False else None
        A = results[a]["_plan"]
        B = results[b]["_plan"]
...
def _wall_diffs(ra, rb, tol: float = 0.35):
    ...
    for m, u, L in A:
        best, bi = None, -1
        for i, (m2, u2, _L2) in enumerate(B):
