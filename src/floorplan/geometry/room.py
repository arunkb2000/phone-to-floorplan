...
def orient_frames(frames: list[Frame], geoms: list[FrameGeom | None], tier: str) -> list[FrameGeom | None]:
    ...
    corner_i = 0
    prev_view = None
    for i, (fr, g) in enumerate(zip(frames, geoms, strict=False)):
        if g is None:
            continue
        if fr.pose is not None:
        ...
        if fr.role == "entry":
            ...
            k = best_rotation(g, CORNER_VIEWS[corner_i % 4])
            corner_i += 1
...
def fuse_room(frames: list[Frame], geoms: list[FrameGeom | None], key: str, label: str, tier: str,
    ...
    for g in geoms:
        if g is not None and np.isfinite(g.cam_height) and np.isfinite(g.ceil_above):
            ...
            ch_v.append(h)
            ch_s.append(g.depth_rel_err * h * 0.7 + 0.008)
    ...
    if not np.isfinite(ch):
        ...
        if hs and cs:
            ...
            chs = float(np.hypot(0.05, 0.03 * ch))
            chn = min(len(hs), len(cs))
    ...
    if cam_xy is not None:
        for g, c in zip(geoms, cam_xy, strict=False):
        ...
        corner_i = 0
        for fr, g in zip(frames, geoms, strict=False):
            if g is None:
                continue
            rel = g.depth_rel_err
            ci = None
    ...
    lxs = max(lxs, floor_rel * lx)
    lys = max(lys, floor_rel * ly)
    if not np.isfinite(ch):
        ch, chs = 2.7, 0.25
...
def _accumulated_openings(frames, geoms, room: RoomEstimate, cam_xy, x0, y0) -> list[Opening]:
    ...
    for i, (_fr, g) in enumerate(zip(frames, geoms, strict=False)):
        ...
        for d, w in g.walls.items():
            ...
            for k in ("on", "cross", "near"):
                acc[letter][k + "_u"].append(f(uz[k + "_u"]))
                acc[letter][k + "_z"].append(uz[k + "_z"])
    out = []
    ...
    for i, (fr, g) in enumerate(zip(frames, geoms, strict=False)):
        ...
        for d, w in g.walls.items():
            ...
            for op in wall_openings(g.P, d, w["dist"], g.cam_height):
                ...
                if np.isfinite(base):
                    lo_ = ref + sgn * (base + op["u0"])
                    hi_ = ref + sgn * (base + op["u1"])
