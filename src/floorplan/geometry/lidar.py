...
def wall_observations(geoms, cams):
    ...
    obs = []
    for i, (g, c) in enumerate(zip(geoms, cams, strict=False)):
        if g is None:
            continue
        for d, w in g.walls.items():
            ax = 0 if d[1] == "x" else 1
...
def rooms_from_cells(geoms, cams, X, Y, min_frames=5):
    ...
    xs = np.array([L["coord"] for L in X])
    ys = np.array([L["coord"] for L in Y])
...
def room_heights_from_points(frames, geoms, cams, stride: int = 3, bounds: dict | None = None,
    ...
    zf, zc = [], []
    for k, (fr, g, c) in enumerate(zip(frames, geoms, cams, strict=False)):
        ...
        zf.append(z[ok & up])
        zc.append(z[ok & down])
    zf = np.concatenate(zf) if zf else np.array([])
    zc = np.concatenate(zc) if zc else np.array([])
...
def global_floor_z(frames, geoms, stride: int = 4) -> float | None:
    zs = []
    for k, (fr, g) in enumerate(zip(frames, geoms, strict=False)):
        if g is None or g.P is None or (k % stride):
            continue
