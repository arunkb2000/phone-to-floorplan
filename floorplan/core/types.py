...
class Frame:
    ...
    depth: Optional[np.ndarray] = None   # H x W float32 metres (same res as rgb after resize) or None
    conf: Optional[np.ndarray] = None    # H x W uint8 (0 low, 1 med, 2 high) or None
    pose: Optional[np.ndarray] = None    # 4 x 4 camera-to-world, or None
    t: float = 0.0                       # seconds
...
class Plane:
    ...
    n_inliers: int = 0
    rms: float = 0.0
    points: Optional[np.ndarray] = None  # inlier points (N x 3), kept for openings/damage
...
class Opening:
    ...
    sill: Optional[Measurement] = None
