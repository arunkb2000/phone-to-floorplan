#!/usr/bin/env bash
# Regenerate every reported number from the raw captures in Dataset/.
#
...
uv run python scripts/split_capture.py Dataset/single_scan_with_ceiling --name flatA
uv run python scripts/make_rgb_tiers.py Dataset/single_scan_with_ceiling --name flatA

echo "== reference ground truth"
for d in single_room single_scan_floor_only single_scan_with_ceiling; do
  uv run python scripts/make_reference_gt.py "Dataset/$d" --out "benchmark/ground_truth/$d.yaml"
done
...
echo
echo "done. read fixloop/DIFF.md, fixloop/after/gates.md and docs/report/benchmark.md"
